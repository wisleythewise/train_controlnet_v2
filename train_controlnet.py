...
def log_validation(
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        transform = transforms.Compose([
            transforms.Resize((int(360) , int(640))),  # Resize the image to 512x512 pixels
...
def save_model_card(repo_id: str, image_logs=None, base_model=str, repo_folder=None):
    ...
    if image_logs is not None:
        ...
        for i, log in enumerate(image_logs):
            ...
            # Check if validation_image is a tensor and convert it
            if isinstance(validation_image, torch.Tensor):
                # Assuming the tensor is in CxHxW format and needs to be normalized

                validation_image = to_pil_image(validation_image.byte())  # Convert tensor to PIL Image
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    def tokenize_captions(examples, is_train=True):
        ...
        for caption in examples[caption_column]:
            if random.random() < args.proportion_empty_prompts:
                ...
                captions.append("A driving scene")
    ...
    image_factor = 360 / 512                
    image_transforms = transforms.Compose(
        [
            transforms.Resize((int(360) , int(640))),
            transforms.ToTensor(),
